...
private boolean title = true;
    ...
    public int checkSpace(int x) {
      // puts piece in the bottom most open space
        for(int i = 5; i >= 0; i--) {
    ...
    public void checkWin() {
      // checks for win
        ...
        }// returns tie if not winner after all the spaces are filled
        if(turn >= 42 && winner == -1)
            winner = 0;
    }
    ...
    public void mouseReleased() {
      //sets pieces to correct color when dropped
        if(!title && winner == -1) {
            if(player1Turn) {
                if(mouseX > 10 && mouseX < 70)
                    if(checkSpace(40) != -1) {
